...
class Application(tk.Frame):
    ...
    def create_menubar(self):
        ...
        menu1.add_command(label='start up', command=self.start_onmyoji)
        menu1.add_command(label='drop out', command=self.stop_onmyoji)
        ...
        menu3.add_command(label='info', command=self.say_hi)
    ...
    def create_client(self):
        ...
        self.client['value'] = ('Steam', 'MUMU Simulator-1136 * 640 (100% Zoom)')
    ...
    def create_section(self):
        ...
        self.section.add(self.frame0, text='Royal soul')

        # Create tab 2 --- 灵
        self.frame1 = tk.Frame(self.section)
        self.section.add(self.frame1, text='Royal spirit')
    ...
    def create_frame0(self):
        ...
        # Game script
        submode = tk.LabelFrame(self.frame0, text='Copy (please lock lineup)')
        submode.pack(padx=5, pady=5, fill=tk.BOTH, expand=True)
        tk.Radiobutton(submode, text='Hachikawa large snake', variable=self.run_submode,
                       value=0).grid(row=0, column=0, sticky=tk.W)
        tk.Radiobutton(submode, text='Industry fire', variable=self.run_submode,
                       value=1).grid(row=0, column=1, sticky=tk.W)
        tk.Radiobutton(submode, text='Himself', variable=self.run_submode,
                       value=2).grid(row=1, column=0, sticky=tk.W)

        # Marker god
        mitama_mark = tk.Frame(self.frame0, padx=5, pady=5)
        mitama_mark.pack(fill=tk.X, expand=True)
        tk.Label(mitama_mark, text='Tagnail:').pack(side=tk.LEFT)
        self.mitama_team_mark = ttk.Combobox(mitama_mark, width=10)
        self.mitama_team_mark['value'] = (
            'Not marked', 'First type god', '2nd style', '3rd style god', '4th style god', '5th style god')
    ...
    def create_frame1(self):
        ...
        text.insert(tk.END, 'Select the Royal Spirit and the number of layers to be hit, click the Start button.')
    ...
    def create_frame2(self):
        ...
        # Dog food setting
        food = tk.LabelFrame(self.frame2, text='Replace the dog food (do not lock lineup)')
        ...
        tk.Label(food, text='Single / player').grid(row=0, column=3, sticky=tk.W)
        ...
        # Change dog food setting
        tk.Checkbutton(self.frame2, text='Change dog grain drag and drop god progress bar, progress:',
        ...
        self.cmb['value'] = ('Replace material', 'Replace N card', 'Replace the R card')
        self.cmb.pack(fill=tk.X, padx=2)
        self.cmb.current(self.change_shikigami)
        self.cmb.config(state='readonly')

        # 打BOSS设置
        tk.Checkbutton(self.frame2, text='Play BOSS after the end',
    ...
    def create_frame3(self):
        ...
        text.insert(
            tk.END, 'Source：%s\n\n' % (self.source_url))
        text.insert(
            tk.END, 'If you feel that the script moves too slow, please go to the Advanced Menu Custom Delay.')
        text.config(state=tk.DISABLED)
    ...
    def create_times(self):
        ...
        Number of games
        '''
        times = tk.LabelFrame(self.main_frame1, text='Number setting')
        times.pack(padx=5, fill=tk.X, anchor=tk.W)
        timeframe1 = tk.Frame(times)
        timeframe1.pack(anchor=tk.W)
        tk.Label(timeframe1, text='Number of games (0 = countless times):').pack(side=tk.LEFT)
    ...
    def create_advance(self):
        ...
        tk.Checkbutton(advance, text='Overseas automatically close yin yang',
                       variable=self.watchdog_enable).pack(anchor=tk.W)
        frame = tk.Frame(advance)
        frame.pack(anchor=tk.W)
        tk.Label(frame, text='  Screen timeout (second):').grid(row=0, column=0)
        tk.Entry(frame, textvariable=self.max_win_time,
                 width=5).grid(row=0, column=1)
        tk.Label(frame, text='  Operation timeout (second):').grid(row=1, column=0)
    ...
    def create_command(self):
        ...
        tk.Button(button_area, text='drop out',
    ...
    def help(self):
        ...
        Q = tk.messagebox.askyesno(
            "Instructions for use", 'For details, please refer to%s\n\nIs it accessible?' % (self.url))
        if Q:
    ...
    def donate(self):
        ...
        Q = tk.messagebox.askyesno(
            "Donate ", 'Donation.\n\nGo to the donation?')
        if Q:
            webbrowser.open('https://streamlabs.com/isomizer/tip')
...
class DelayDialog(tk.Toplevel):
    def __init__(self, parent):
        ...
        text.pack(expand=True, fill=tk.BOTH)
        text.insert(tk.END, '1- Total 5 delay, the script randomly selects the first level from the 1-5 delay as the main delay,\
At the same time, the first level is selected in the 1-3 delay as a secondary delay.Based on this basis by random coefficients.\n\n')
        text.insert(tk.END, '2- Fresh selection every 5 minutes, calculate unit milliseconds.\n\n')
        text.insert(tk.END, '3- Main delay is used for screenshots, knowledge, and the like, and the secondary delay is mainly used for settlement.\n\n')
        text.insert(tk.END, '4- Don\'t entangle why it doesn\'t turn this form every time, the parameter is delay.json, restart is valid.\n\n')
        text.config(state=tk.DISABLED)

        # parameter settings
        row2 = tk.Frame(self)
        row2.pack(fill=tk.X)
        tk.Label(row2, text='Level: ').grid(row=0, column=0)
        ...
        tk.Label(row2, text='Secondary: ').grid(row=1, column=0)
        ...
        tk.Label(row2, text='Level three: ').grid(row=2, column=0)
        ...
        tk.Label(row2, text='Four-level: ').grid(row=3, column=0)
        ...
        tk.Label(row2, text='Five-level: ').grid(row=4, column=0)
        ...
        tk.Button(row3, text='determine', command=self.confirm).grid(row=0, column=0)
